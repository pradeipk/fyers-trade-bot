...
public class AdjustingStraddle implements FyersSocketDelegate, FyerBotInterface {
	...
	private void analyzePremiums() {
		...
		if (System.currentTimeMillis() - printTimer > 60000) {
			...
			printBuilder.append("\nGain/Loss in premium for (+ive delta is gain in your favour.)").append(" \n")
			.append("Delta CE       ").append(" : ").append(df.format(delta_CE))
			.append(" ( ").append(df.format(start_ce)).append(" --> ").append(df.format(ce)).append(" )\n")
			.append("Delta PE       ").append(" : ").append(df.format(delta_PE))
			.append(" ( ").append(df.format(start_pe)).append(" --> ").append(df.format(pe)).append(" )\n")
			.append("Delta Premium  ").append(" : ").append(df.format(delta_Combined_premium))
			.append(" ( ").append(df.format(startPremium)).append(" --> ").append(df.format(current_combinedPremium)).append(" )\n")
			.append("Trailing SL    ").append(" : ").append(df.format(combinedPremiumLimit)).append("\n")
	...
	private void OnScripeCallback(JSONObject strikePrice) {
		...
		if (combinedPremiumLimit < current_combinedPremium) {
			...
			if (!exitDonePE && delta_PE < 0) {
				...
				exitPositionList.add(positionID);
	...
	private void InitializeAndUpdatePremium(MarketData data) {
		...
		if(ce == 0.0 || pe == 0.0 || niftyIndex == 0.0) {
			...
			positionDTO = fyerOperations.getPositionsBySymbols(pool.subscriptionlist);			
